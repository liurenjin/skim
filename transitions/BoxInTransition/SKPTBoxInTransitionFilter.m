...
#import "SKPTBoxInTransitionFilter.h"
#import <Foundation/Foundation.h>
#import <ApplicationServices/ApplicationServices.h>

@implementation SKPTBoxInTransitionFilter

static CIKernel *_SKPTBoxInTransitionFilterKernel = nil;

- (id)init
{
    if(_SKPTBoxInTransitionFilterKernel == nil)
    {
		NSBundle    *bundle = [NSBundle bundleForClass:NSClassFromString(@"SKPTBoxInTransitionFilter")];
		NSStringEncoding encoding = NSUTF8StringEncoding;
		NSError     *error = nil;
		NSString    *code = [NSString stringWithContentsOfFile:[bundle pathForResource:@"SKPTBoxInTransitionFilterKernel" ofType:@"cikernel"] encoding:encoding error:&error];
		NSArray     *kernels = [CIKernel kernelsWithString:code];

		_SKPTBoxInTransitionFilterKernel = [[kernels objectAtIndex:0] retain];
    }
    return [super init];
}
...
- (CIImage *)outputImage
{
    ...
    return [self apply:_SKPTBoxInTransitionFilterKernel arguments:arguments options:options];
}
