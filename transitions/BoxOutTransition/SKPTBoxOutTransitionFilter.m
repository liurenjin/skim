...
#import "SKPTBoxOutTransitionFilter.h"
#import <Foundation/Foundation.h>
#import <ApplicationServices/ApplicationServices.h>

@implementation SKPTBoxOutTransitionFilter

static CIKernel *_SKPTBoxOutTransitionFilterKernel = nil;

- (id)init
{
    if(_SKPTBoxOutTransitionFilterKernel == nil)
    {
		NSBundle    *bundle = [NSBundle bundleForClass:NSClassFromString(@"SKPTBoxOutTransitionFilter")];
		NSStringEncoding encoding = NSUTF8StringEncoding;
		NSError     *error = nil;
		NSString    *code = [NSString stringWithContentsOfFile:[bundle pathForResource:@"SKPTBoxOutTransitionFilterKernel" ofType:@"cikernel"] encoding:encoding error:&error];
		NSArray     *kernels = [CIKernel kernelsWithString:code];

		_SKPTBoxOutTransitionFilterKernel = [[kernels objectAtIndex:0] retain];
    }
    return [super init];
}
...
- (CIImage *)outputImage
{
    ...
    return [self apply:_SKPTBoxOutTransitionFilterKernel arguments:arguments options:options];
}
