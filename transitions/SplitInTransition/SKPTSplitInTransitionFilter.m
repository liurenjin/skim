...
#import "SKPTSplitInTransitionFilter.h"
#import <Foundation/Foundation.h>
#import <ApplicationServices/ApplicationServices.h>

@implementation SKPTSplitInTransitionFilter

static CIKernel *_SKPTSplitInTransitionFilterKernel = nil;

- (id)init
{
    if(_SKPTSplitInTransitionFilterKernel == nil)
    {
		NSBundle    *bundle = [NSBundle bundleForClass:NSClassFromString(@"SKPTSplitInTransitionFilter")];
		NSStringEncoding encoding = NSUTF8StringEncoding;
		NSError     *error = nil;
		NSString    *code = [NSString stringWithContentsOfFile:[bundle pathForResource:@"SKPTSplitInTransitionFilterKernel" ofType:@"cikernel"] encoding:encoding error:&error];
		NSArray     *kernels = [CIKernel kernelsWithString:code];

		_SKPTSplitInTransitionFilterKernel = [[kernels objectAtIndex:0] retain];
    }
    return [super init];
}
...
- (CIImage *)outputImage
{
    ...
    return [self apply:_SKPTSplitInTransitionFilterKernel arguments:arguments options:options];
}
